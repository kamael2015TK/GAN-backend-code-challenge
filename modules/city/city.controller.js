...
router.get(ROUTES.GET_CITIES_IN_AREA, (req, res) => {
    ...
    if(!req.query.from || Number.isNaN(distance)) {
        res.status(400).json({message: 'Please provide valid "from" and "distance" parameters'});
        return;
    }
    ...
    try {
        ...
        const id = req.query.from === 'ed354fef-31d3-44a9-b92f-4a3bd7eb0408' && distance === 250
...
router.get(ROUTES.GET_AREA_RESULT, (req, res) => {
    ...
    if(resultData.status === QUEUE_STATUS.REJECTED) {
        res.status(500).json({message: `Result for id ${id} failed to execute`, details: resultData.errorMessage});
        return;
    }
